...
c["Autosave Directory"] = f"D:\\Service\\{loc_time.tm_year}-{loc_time.tm_mon}-{loc_time.tm_mday}"
...
for i in range(100):
    c.start_acquisition(1) # Acquire one image (This is non-blocking and should run very fast)
...
plt.plot(np.linspace(histogram.min, histogram.max, histogram.bins),histogram.data)
