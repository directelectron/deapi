...
class TestUtils:
    ...
    def test_image_adjust_gain(self, client, binx, offset_y, flip_horizontal, flip_vertical):
        ...
        client.take_gain_reference(100, target_electrons_per_pixel= 100) # ignore warning for testing
        time.sleep(1)
        client["Binning Y"] = 1
        client["Binning X"] = binx
        client["Crop Offset Y"] = offset_y
        client["Crop Size Y"] = 1024 - offset_y
        client["Image Processing - Flip Horizontally"] = "On" if flip_horizontal else "Off"
        client["Image Processing - Flip Vertically"] = "On" if flip_vertical else "Off"

        client["Image Processing - Flatfield Correction"] = "Dark and Gain" # engineering property...
        ...
        assert  np.not_equal(final_image, movie).any(), "Movie should not be equal to final image after gain adjustment"
        ...
        np.testing.assert_array_almost_equal(final_image[0, 0], adjusted_movie_final, decimal=-1 )
    ...
    def test_get_gain(self, client):
        ...
        assert client["Reference - Integrating Gain"] != prev_gain, "Gain should have been applied"
        ...
        final_img = np.memmap(f, offset = 1024, dtype= np.float32, mode='r', shape=(1024, 1024))
